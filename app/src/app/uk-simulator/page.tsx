    ...
    <div className="space-y-5">
      <Section title="You">
        ...
        <RangeField
          ...
          min={Math.max(25, input.current_age + 1)}
          max={120}
          format={(v) => String(v)}
        />
        <PairedFields>
      ...
      <Section title="Spending & income">
        ...
        />
        <LogRangeField
          label="Employment income (until retirement)"
        ...
        {employmentIncome > 0 && (
          <RangeField
            ...
            value={input.retirement_age ?? input.current_age + 1}
...
function HeroAnswer({
  ...
  return (
    <div
      className={`relative rounded-[var(--radius-lg)] border border-[var(--color-border-light)] bg-white p-6 shadow-[var(--shadow-sm)] transition-opacity md:p-8 ${
        isRunning && result ? "opacity-90" : ""
      }`}
    >
