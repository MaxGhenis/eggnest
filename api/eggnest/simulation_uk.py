...
def _iterate_years(
    ...
    for year_idx in range(n_years):
        ...
        # Pre-retirement contributions: route a share of gross earnings into
        # SIPP / ISA. Auto-enrolment default is 8 % of gross; we apply the
        # split from the simulator inputs. Contributions happen out of gross
        # pay regardless of whether spending was covered.
        contributions_total = employment * inputs.savings_rate
        sipp_contrib = contributions_total * inputs.sipp_contribution_share
        isa_contrib = contributions_total - sipp_contrib
        state.sipp = state.sipp + sipp_contrib
        state.isa = state.isa + isa_contrib
